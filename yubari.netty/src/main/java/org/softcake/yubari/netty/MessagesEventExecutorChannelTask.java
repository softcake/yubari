...
abstract class MessagesEventExecutorChannelTask implements OrderedThreadPoolExecutor.OrderedRunnable {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MessagesEventExecutorChannelTask.class);
    ...
    MessagesEventExecutorChannelTask(final ChannelHandlerContext ctx,
    ...
    public void run() {
        final ISessionStats stats = this.session.getSessionStats();

        final boolean
            canProcessCurrentDroppable
            = this.protocolHandler.getDroppableMessageHandler().canProcessDroppableMessage(this.message, this.currentDropableMessageTime);
        if (!canProcessCurrentDroppable) {
            this.session.getDroppedMessageCounter().incrementAndGet();
...
//        }
    ...
    public Object getOrderKey() {
        ...
        return this.session.getConcurrencyPolicy().getConcurrentKey(this.message);
    }

    void executeInExecutor(final ListeningExecutorService executor, final ISessionStats stats) {
        ...
        if (checkError) {
            ...
            final Runnable runnable = this.getRunnableForErrorOrWarningCheck(stats,
                                                                             future,
            ...
            if (lastExecutionWarningOrErrorTime + ERROR_TIME_OFFSET < nanoTime && isSet) {
                ...
                if (isError) {
                    LOGGER.error("[{}] Event did not execute in timeout time [{}] and is still executing, "
                                 ...
                                 this.session.getEventExecutionErrorDelay(),
                                 postponeInterval,
                                 shortMessage);
                } else {
                ...
                this.protocolHandler.checkAndLogEventPoolThreadDumps();

                if (stats != null) {
                    stats.messageProcessingPostponeError(now, postponeInterval, this.message);
                }

                future.addListener(this.getRunnableListenerForWarnOrErrorMessage(isError, startTime),
    ...
    private ListenableFuture<?> executeInExecutor(final ListeningExecutorService executor, final boolean withFuture) {
        ...
        try {
            ...
            if (!this.session.getLogEventPoolThreadDumpsOnLongExecution().get()) {
            ...
            this.delayedExecutionTask = new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        if (future != null) {
                            future.setFuture(executor.submit(MessagesEventExecutorChannelTask.this));
                        } else {
                            executor.execute(MessagesEventExecutorChannelTask.this);
                        }
